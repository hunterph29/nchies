...
function initStylishCursor() {
  ...
  function checkIsDark(el) {
    ...
    // When dark mode is active, almost everything is dark
    // Exception: award cards (behance-card, idesign-card, muzli-card) stay light
    const isDarkMode = document.body.classList.contains('dark-mode');
    if (isDarkMode) {
      // These cards keep their light backgrounds even in dark mode
      if (el.closest('.behance-card, .idesign-card, .muzli-card')) {
        return false;
      }
      // Everything else in dark mode is dark
      return true;
    }

    // Light mode: original detection logic
...
(function initDarkMode() {
  ...
  checkbox.addEventListener('change', () => {
    ...
    // Remove after transition completes so it doesn't interfere with scroll/hover animations
